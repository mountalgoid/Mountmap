...
class CanvasPainter extends CustomPainter {
  ...
  void _drawConnection(Canvas canvas, Size size, NodeModel startNode, NodeModel endNode, bool isCross, Map<String, NodeModel> nodeMap) {
    ...
    // Estimasi tinggi berdasarkan jumlah tag dan deskripsi
    double getEstimatedHeight(NodeModel n) {
      double h = 60.0; // Base height
      if (n.description != null && n.description!.isNotEmpty) h += 20.0;
      if (n.labels.isNotEmpty) h += (n.labels.length / 2).ceil() * 25.0 + 10.0;
      return h;
    }
    ...
    const double nodeVisibleW = 160.0; // Estimasi lebar rata-rata node

    // Titik jangkar yang menjorok masuk ke body agar seamless
    double startX = isChildOnRight ? (startNode.position.dx + nodeVisibleW - 30) : (startNode.position.dx + 30);
    double endX = isChildOnRight ? (endNode.position.dx + 30) : (endNode.position.dx + nodeVisibleW - 30);

    // Posisi absolut dengan padding global 20
    Offset start = Offset(startX + 20, startNode.position.dy + startH * 0.5 + 20);
    Offset end = Offset(endX + 20, endNode.position.dy + endH * 0.5 + 20);
    ...
    if (isConnectedToSelected) {
      ...
      paint.shader = ui.Gradient.linear(
        ...
        [
          MountMapColors.violet.withValues(alpha: 0.3),
          MountMapColors.teal.withValues(alpha: 0.3),
        ],
      );
    }
  ...
  void _drawArrowHead(Canvas canvas, Offset controlPoint, Offset endPoint, Paint paint) {
    ...
    final arrowPaint = Paint()
      ..color = paint.shader == null ? paint.color : Colors.white
      ..shader = paint.shader
