...
class ChartEnginePainter extends CustomPainter {
  ...
  void _drawPlaceholder(Canvas canvas, Size size, String message) {
    final tp = TextPainter(
      text: TextSpan(text: message, style: const TextStyle(color: Colors.white24, fontSize: 14)),
  ...
  void _drawAlluvial(Canvas canvas, Size size, List<List<String>> table) {
    ...
    levelGroups.forEach((lvl, nodes) {
      ...
      double totalVal = levelTotals[lvl]!;
      ...
      for (var n in nodes) {
        ...
        double h = totalVal > 0 ? (val / totalVal) * (chartHeight * 0.8) : chartHeight / nodes.length;
    ...
    for (var conn in connections) {
      ...
      double srcTotal = nodeTotalOut[src]!;
      double tgtTotal = nodeTotalIn[tgt]!;
      ...
      if (isHovered || isSelected || visualSettings['intensity']! > 0.6) {
  ...
  void _drawChord(Canvas canvas, Size size, List<List<String>> table) {
    ...
    double availableAngle = 2 * math.pi - (sortedNames.length * gap);

    // Draw Outer Segments
    for (var name in sortedNames) {
      double sweep = (nodeTotals[name]! / totalVal) * availableAngle;
    ...
    for (var flow in flows) {
      ...
      double sStart = nodeStartAngles[src]! + (currentOutOffset[src] ?? 0);
      double sSweep = (val / totalVal) * availableAngle;
      currentOutOffset[src] = (currentOutOffset[src] ?? 0) + sSweep;

      double tStart = nodeStartAngles[tgt]! + (currentInOffset[tgt] ?? 0);
      double tSweep = (val / totalVal) * availableAngle;
      ...
      // Flow to target with control points near center
      path.cubicTo(center.dx * 1.1, center.dy * 1.1, center.dx * 0.9, center.dy * 0.9, t1.dx, t1.dy);
      path.arcToPoint(t2, radius: Radius.circular(innerRadius));
      path.cubicTo(center.dx * 0.9, center.dy * 0.9, center.dx * 1.1, center.dy * 1.1, s1.dx, s1.dy);
      path.close();
  ...
  void _drawContour(Canvas canvas, Size size, List<List<String>> table) {
    ...
    double chartW = size.width - 120, chartH = size.height - 100;
    Offset origin = const Offset(60, 40);

    // 1. Generate Grid Values using IDW
    int resX = (visualSettings['thickness'] ?? 40.0).toInt();
    int resY = (resX * 0.75).toInt();
    ...
    for (int i = 0; i <= resX; i++) {
      for (int j = 0; j <= resY; j++) {
        ...
        for (var p in points) {
          double d = math.sqrt(math.pow(vx - p['x']!, 2) + math.pow(vy - p['y']!, 2));
    ...
    for (int i = 0; i < resX; i++) {
      for (int j = 0; j < resY; j++) {
        ...
        double norm = (avgZ - minZ) / (maxZ - minZ);
    ...
    for (int i = 0; i < points.length; i++) {
      var p = points[i];
      final bool isSelected = selectedRowIndex == i + 1;
      double px = origin.dx + ((p['x']! - minX) / (maxX - minX)) * chartW;
      double py = origin.dy + ((p['y']! - minY) / (maxY - minY)) * chartH;
      ...
      if (showLabels) _drawText(canvas, p['z']!.toStringAsFixed(0), Offset(px, py - 12), Colors.white70, true);
    }
  ...
  void _drawMultiLevelPie(Canvas canvas, Size size, List<List<String>> table) {
    ...
    double calculateTotal(String node) {
      final children = tree[node] ?? [];
      if (children.isEmpty) return values[node] ?? 0;
      double sum = children.fold(0.0, (s, c) => s + calculateTotal(c));
      values[node] = sum;
      return sum;
    }
    ...
    void drawRing(String node, double startAngle, double sweep, int depth) {
      ...
      final double motionScale = visualSettings['intensity']! > 0.6 ? (math.sin(DateTime.now().millisecondsSinceEpoch / 1500 + depth) * 0.02) : 0;
      final path = Path();
      ...
      for (var child in children) {
        ...
        double childSweep = (childVal / values[node]!) * sweep;
  ...
  void _drawPareto(Canvas canvas, Size size, List<List<String>> table) {
    ...
    for (int i = 1; i < table.length; i++) {
      ...
      items.add({'name': table[i][0], 'val': v});
      total += v;
    }
    if (total == 0) total = 1;
  ...
  void _drawTaylor(Canvas canvas, Size size, List<List<String>> table) {
    ...
    double chartSize = math.min(size.width - p * 2.5, size.height - p * 2);
    double maxSD = 2.0;
  ...
  double _worstAspectRatio(List<Map<String, dynamic>> row, double w, double total, double totalArea) {
    ...
    return math.max(math.pow(w, 2) * maxArea / math.pow(rowArea, 2), math.pow(rowArea, 2) / (math.pow(w, 2) * minArea));
  }
  ...
  Rect _layoutRow(List<Map<String, dynamic>> row, Rect rect, Canvas canvas, double total) {
    ...
    double rowWidth = vertical ? rect.width : rowArea / rect.height;
    double rowHeight = vertical ? rowArea / rect.width : rect.height;
  ...
  void _drawStreamGraph(Canvas canvas, Size size, List<List<String>> table) {
    ...
    for (int j = 0; j < n; j++) {
      double sum = 0;
      for (int i = 0; i < m; i++) {
        sum += (m - i - 0.5) * seriesMap[categories[i]]![j];
      }
      baseline[j] = (-sum / m) * wiggleFactor;
    }
  ...
  void _drawSankey(Canvas canvas, Size size, List<List<String>> table) {
    ...
    for (var conn in connections) {
      ...
      double h1 = (val / nodeTotalOut[src]!) * srcRect.height;
      double h2 = (val / nodeTotalIn[tgt]!) * tgtRect.height;
      ...
      if (isSelected || visualSettings['intensity']! > 0.5) {
        final pPaint = Paint()..color = Colors.white.withValues(alpha: 0.5)..style = PaintingStyle.fill;
        double time = DateTime.now().millisecondsSinceEpoch / 2000.0;
        for (int i = 0; i < 3; i++) {
  ...
  void _drawRose(Canvas canvas, Size size, List<List<String>> table) {
    ...
    for (int i = 0; i < items.length; i++) {
      ...
      final double motionOffset = visualSettings['intensity']! > 0.7 ? (math.sin(DateTime.now().millisecondsSinceEpoch / 1000 + i) * 5) : 0;
  ...
  void _drawDataTable(Canvas canvas, Size size, List<List<String>> table) {
    ...
    for (int i = 0; i < table.length; i++) {
      ...
      if (i == 0) {
        ...
        canvas.drawRect(Rect.fromLTWH(0, y, size.width, rowHeight), Paint()..color = Colors.white.withValues(alpha: 0.02));
      }

      for (int j = 0; j < table[i].length; j++) {
        double x = j * colWidth;
        _drawText(canvas, table[i][j], Offset(x + colWidth / 2, y + rowHeight / 2), i == 0 ? MountMapColors.teal : Colors.white70, true);

        // Vertical dividers
        if (j > 0) {
          canvas.drawLine(Offset(x, y + 5), Offset(x, y + rowHeight - 5), Paint()..color = Colors.white.withValues(alpha: 0.05));
        }
      }

      // Horizontal divider
      canvas.drawLine(Offset(0, y + rowHeight), Offset(size.width, y + rowHeight), Paint()..color = Colors.white.withValues(alpha: 0.05));
    }
  }
  ...
  void _applyEffects(Canvas canvas, dynamic shape, Paint basePaint) {
    ...
    if (sInten > 0) {
      final sPaint = Paint()
        ..color = glowColor != Colors.transparent ? glowColor.withValues(alpha: sInten) : Colors.black.withValues(alpha: sInten * 0.5)
  ...
  int? _hitTestMultiLevelPie(Offset pos, Size size, List<List<String>> table) {
    ...
    double calculateTotal(String node) {
      final children = tree[node] ?? [];
      if (children.isEmpty) return values[node] ?? 0;
      double sum = children.fold(0.0, (s, c) => s + calculateTotal(c['name']));
      values[node] = sum;
      return sum;
    }
    calculateTotal(root);
  ...
  int? _hitTestPareto(Offset pos, Size size, List<List<String>> table) {
    ...
    for (int i = 0; i < items.length; i++) {
      double h = (items[i]['val'] / maxBar) * chartH;
      double x = pL + i * stepW + (stepW - barW) / 2;
      double y = pT + chartH - h;
  ...
  int? _hitTestStreamGraph(Offset pos, Size size, List<List<String>> table) {
    ...
    final categories = seriesMap.keys.toList();
    int m = categories.length; int n = timePoints.length;
    double dx = size.width / (n - 1);
    double heightScale = visualSettings['intensity'] ?? 1.0;
    double wiggleFactor = visualSettings['smoothing'] ?? 0.5;
    ...
    for (int j = 0; j < n; j++) {
      double sum = 0;
      for (int i = 0; i < m; i++) { sum += (m - i - 0.5) * seriesMap[categories[i]]![j]; }
      baseline[j] = (-sum / m) * wiggleFactor;
    }
  ...
  int? _hitTestHyperbolic(Offset pos, Size size, List<List<String>> table) {
    ...
    void findHit(String name, double angle, double sweep, double r) {
      double d = diskRadius * (r / (r + 1.5));
      Offset p = center + Offset(math.cos(angle) * d, math.sin(angle) * d);
      double nodeSize = (baseNodeSize / (r + 1)).clamp(2.0, baseNodeSize);
  ...
  int? _hitTestRose(Offset pos, Size size, List<List<String>> table) {
    ...
    double r = ((double.tryParse(table[index + 1][1]) ?? 0) / maxVal) * maxRadius;
  ...
  int? _hitTestContour(Offset pos, Size size, List<List<String>> table) {
    ...
    for (int i = 1; i < table.length; i++) {
      double x = double.tryParse(table[i][0]) ?? 0, y = double.tryParse(table[i][1]) ?? 0;
      double px = origin.dx + ((x - minX) / (maxX - minX)) * chartW;
      double py = origin.dy + ((y - minY) / (maxY - minY)) * chartH;
