...
class _MountMapCanvasState extends State<MountMapCanvas> with SingleTickerProviderStateMixin {
        ...
        child: Column(
          ...
          children: [
            ...
            GridView.count(
              ...
              children: [
                _menuIconBtn(Icons.edit_note_rounded, "Rename", Colors.blueAccent, 
                  () => _handleRename(node, provider)),
                
                _menuIconBtn(Icons.account_tree_rounded, "Branch", Colors.tealAccent, () {
                  provider.addChildPeak(node.id); Navigator.pop(context);
                }),

                _menuIconBtn(Icons.description_outlined, "Desc", Colors.cyanAccent,
                  () => _handleDescription(node, provider)),

                _menuIconBtn(Icons.control_point_duplicate_rounded, "Duplicate", Colors.purpleAccent,
                  () => _showRecursiveActionDialog(context, "Duplicate", (inc) => provider.duplicateNode(node.id, includeChildren: inc))),

                _menuIconBtn(Icons.content_cut_rounded, "Cut", Colors.pinkAccent,
                  () => _showRecursiveActionDialog(context, "Cut", (inc) => provider.cutNode(node.id, includeChildren: inc))),

                _menuIconBtn(Icons.copy_rounded, "Copy", Colors.grey,
                  () => _showRecursiveActionDialog(context, "Copy", (inc) => provider.copyNode(node.id, includeChildren: inc))),

                if (provider.hasClipboard)
                   _menuIconBtn(Icons.paste_rounded, "Paste", Colors.amberAccent, () {
                     provider.pasteNodeAsChild(node.id); Navigator.pop(context);
                   })
                else
                   _menuIconBtn(Icons.paste_rounded, "Paste", Colors.white10, () {}),

                _menuIconBtn(Icons.attachment_rounded, "Attach", Colors.indigoAccent, 
                  () => _handleAttachmentsManager(node, provider)),

                _menuIconBtn(Icons.sell_outlined, "Tags", Colors.greenAccent, 
                  () => _handleTags(node, provider)),
                
                _menuIconBtn(Icons.notifications_active_rounded, "Alert", Colors.orangeAccent, 
                  () => _handleAlert(node, provider)),

                _menuIconBtn(Icons.palette_rounded, "Style", Colors.amberAccent,
                  () => _handleStyle(node, provider)),

                _menuIconBtn(Icons.format_list_numbered_rounded, "Number", Colors.blueGrey,
                  () => _handleNumberingMenu(node, provider)),

                _menuIconBtn(Icons.account_tree_rounded, "Flow", Colors.deepOrangeAccent,
                  () => _handleShapeMenu(node, provider)),

                _menuIconBtn(Icons.linear_scale_rounded, "Line", Colors.limeAccent,
                  () => _handleLineLabelMenu(node, provider)),

                _menuIconBtn(Icons.table_chart_rounded, "Data", Colors.lightBlueAccent,
                  () => _handleDiagramData(node, provider)),

                _menuIconBtn(Icons.link_rounded, "Connect", Colors.greenAccent, () {
                ...
                child: node.attachments.isEmpty
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          Icon(Icons.attachment_rounded, size: 40, color: Colors.white.withValues(alpha: 0.1)),
                          const SizedBox(height: 10),
                          const Text("No attachments yet", style: TextStyle(color: Colors.white24)),
                        ],
                      ),
                    )
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Container(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text(item.value, style: const TextStyle(color: Colors.white38, fontSize: 10), maxLines: 1, overflow: TextOverflow.ellipsis),
                            trailing: PopupMenuButton(
                              ...
                              itemBuilder: (context) => [
                                ...
                                if (isLink) const PopupMenuItem(value: 'edit_url', child: Text("Edit URL", style: TextStyle(color: Colors.white))),
      ...
      Column(
        ...
        children: [
          TextField(
            controller: nameCtrl,
            style: const TextStyle(color: Colors.white),
            decoration: const InputDecoration(labelText: "Display Name (Optional)", labelStyle: TextStyle(color: Colors.white54)),
          ),
          const SizedBox(height: 10),
          TextField(
            controller: urlCtrl,
            style: const TextStyle(color: Colors.white),
            decoration: const InputDecoration(labelText: "URL (https://...)", labelStyle: TextStyle(color: Colors.white54), hintText: "https://google.com", hintStyle: TextStyle(color: Colors.white10)),
          ),
        ],
      ), 
      () {
  ...
  void _editAttachmentValue(NodeModel node, MountMapProvider provider, AttachmentItem item) {
    ...
    _showStyledDialog(
      "Edit URL",
      TextField(controller: valCtrl, autofocus: true, style: const TextStyle(color: Colors.white)),
      () {
  ...
  void _handleRename(NodeModel node, MountMapProvider provider) {
    ...
    _showStyledDialog("Rename Node", TextField(controller: c, autofocus: true, style: const TextStyle(color: Colors.white)), () { provider.updatePeak(node.id, text: c.text); Navigator.pop(context); });
  }

  void _handleDescription(NodeModel node, MountMapProvider provider) {
    Navigator.pop(context);
    final c = TextEditingController(text: node.description);
    _showStyledDialog("Branch Description", TextField(
      controller: c,
      maxLines: 4,
      autofocus: true,
      style: const TextStyle(color: Colors.white),
      decoration: const InputDecoration(hintText: "Enter description...", hintStyle: TextStyle(color: Colors.white24)),
    ), () {
      provider.updatePeak(node.id, description: c.text);
      Navigator.pop(context);
    });
  }
    ...
    showModalBottomSheet(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setModalState) => Container(
          ...
          child: Column(
            children: [
              const Text("Table Data Editor", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
              const SizedBox(height: 10),
              Expanded(
    ...
    showModalBottomSheet(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setModalState) => Container(
          ...
          child: Column(
            children: [
              Text(node.shapeType == 'triangle' ? "Pyramid Levels" : "Timeline Items", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
              const SizedBox(height: 10),
              Expanded(
            ...
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text("Manage Tags", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
      ...
      Column(
        ...
        children: [
          ListTile(
            ...
            title: const Text("Just this branch", style: TextStyle(color: Colors.white)),
            subtitle: Text(isDelete ? "Children will be moved to parent" : "Children will not be included", style: const TextStyle(color: Colors.white54, fontSize: 11)),
            onTap: () { Navigator.pop(context); onConfirm(false); Navigator.pop(context); },
          ),
          ListTile(
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
            title: const Text("Include all descendants", style: TextStyle(color: Colors.white)),
            subtitle: const Text("Operate on the entire subtree", style: TextStyle(color: Colors.white54, fontSize: 11)),
            ...
            children: [
              Row(
                ...
                children: [
                  const Row(
                    children: [
                      Icon(Icons.notifications_active_rounded, color: Colors.orangeAccent, size: 22),
                      SizedBox(width: 10),
                      Text("Set Alert", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
                    ],
                  ),
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          backgroundColor: const Color(0xFF1C1E29),
          title: const Row(
            children: [
              Icon(Icons.notifications_active_rounded, color: Colors.orangeAccent),
              SizedBox(width: 10),
              Text("Configure Alert", style: TextStyle(color: Colors.white, fontSize: 16)),
            ],
          ),
          content: SingleChildScrollView(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text("Alert Message (Optional)", style: TextStyle(color: Colors.white70, fontSize: 12)),
                ...
                const Text("Alert Date & Time", style: TextStyle(color: Colors.white70, fontSize: 12)),
                const SizedBox(height: 8),
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text("Auto-Numbering", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 20),
            ListTile(
              leading: const Icon(Icons.account_tree_rounded, color: Colors.tealAccent),
              title: const Text("Number All (Hierarchical)", style: TextStyle(color: Colors.white)),
              onTap: () { provider.applyNumberingAll(); Navigator.pop(context); },
            ),
            ListTile(
              leading: const Icon(Icons.reorder_rounded, color: Colors.amberAccent),
              title: const Text("Number This Subtree", style: TextStyle(color: Colors.white)),
            ...
            ListTile(
              ...
              title: const Text("Number Immediate Children", style: TextStyle(color: Colors.white)),
  ...
  void _handleLineLabelMenu(NodeModel node, MountMapProvider provider) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: MountMapColors.darkCard,
        title: const Text("Connection Label", style: TextStyle(color: Colors.white)),
        content: TextField(
          controller: ctrl,
          style: const TextStyle(color: Colors.white),
          decoration: const InputDecoration(
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text("Flow Chart Design", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 20),
            ...
            children: [
              Row(
                ...
                children: [
                  const Row(
                    children: [
                      Icon(Icons.palette_rounded, color: Colors.amberAccent, size: 22),
                      SizedBox(width: 10),
                      Text("Customize Branch", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
                    ],
                  ),
                  TextButton(
                    onPressed: () {
              ...
              const Text("Body Color", style: TextStyle(color: Colors.white70, fontSize: 12)),
              const SizedBox(height: 10),
              Wrap(
              ...
              SwitchListTile(
                ...
                title: const Text("Gradient Body", style: TextStyle(color: Colors.white, fontSize: 14)),
                subtitle: const Text("Apply consistent Teal/Violet gradient", style: TextStyle(color: Colors.white24, fontSize: 11)),
              ...
              SwitchListTile(
                contentPadding: EdgeInsets.zero,
                title: const Text("Gradient Text", style: TextStyle(color: Colors.white, fontSize: 14)),
              ...
              SwitchListTile(
                contentPadding: EdgeInsets.zero,
                title: const Text("Gradient Icon", style: TextStyle(color: Colors.white, fontSize: 14)),
              ...
              const SizedBox(height: 20),
              const Text("Icon & Indicator Color", style: TextStyle(color: Colors.white70, fontSize: 12)),
              const SizedBox(height: 10),
              Wrap(
                spacing: 12, runSpacing: 12,
                children: textPresets.map((color) => _colorPickerItem(
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  ...
                  Text(t, style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold, decoration: TextDecoration.none)),
                ]),
                const SizedBox(height: 20),
  ...
  Widget _menuIconBtn(IconData i, String l, Color c, VoidCallback t) => InkWell(onTap: t, borderRadius: BorderRadius.circular(15), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: c.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(15)), child: Icon(i, color: c, size: 24)), const SizedBox(height: 8), Text(l, style: const TextStyle(color: Colors.white38, fontSize: 10, fontWeight: FontWeight.w600))]));
  
  Widget _buildMenuHeader(NodeModel n) => Row(children: [const CircleAvatar(backgroundColor: MountMapColors.teal, child: Icon(Icons.hub_rounded, color: Colors.white)), const SizedBox(width: 15), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(n.text, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis), Text(n.parentId == null ? "Root Node" : "Child Node", style: const TextStyle(color: Colors.white24, fontSize: 10))]))]);
      ...
      child: Center(
        child: Container(
          ...
          child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  _navButton(Icons.arrow_back_ios_new_rounded, () {
                  ...
                  _divider(),
                  _navButton(Icons.auto_graph_rounded, () {
                    provider.autoLayout();
                  ...
                  _divider(),
                  _navButton(Icons.more_horiz_rounded, () {
                  ...
                  if (provider.hasClipboard) ...[
                    _divider(),
                    _navButton(Icons.paste_rounded, () {
                      if (provider.selectedNodeId != null) {
                  ...
                  _divider(),
                  _navButton(Icons.arrow_forward_ios_rounded, () {
  ...
  Widget _divider() => Container(width: 1, height: 20, color: Colors.white10, margin: const EdgeInsets.symmetric(horizontal: 8));
  Widget _navButton(IconData icon, VoidCallback onTap, {bool isPrimary = false, bool isAccent = false}) { Color c = Colors.white70; if (isPrimary) c = MountMapColors.teal; if (isAccent) c = Colors.amberAccent; return IconButton(icon: Icon(icon, color: c, size: (isPrimary || isAccent) ? 26 : 22), onPressed: onTap, splashRadius: 24); }

  PreferredSizeWidget _buildAppBar(MountMapProvider provider) {
    final isDark = provider.currentTheme == AppThemeMode.dark;
    return AppBar(
      backgroundColor: isDark ? provider.backgroundColor : Colors.white,
      elevation: 0,
      title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(provider.activeAsset?.title ?? "STRATEGY CANVAS",
          style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, letterSpacing: 1.2, color: isDark ? Colors.white : Colors.black87)),
      ...
      actions: [
        ...
        IconButton(
          icon: Icon(_showOutline ? Icons.grid_view_rounded : Icons.list_alt_rounded, color: Colors.white70),
        ...
        IconButton(icon: const Icon(Icons.search_rounded, color: Colors.white70), onPressed: () => _showSearch(provider)),
        IconButton(icon: const Icon(Icons.center_focus_strong_rounded, color: MountMapColors.teal), onPressed: () => _focusOnNodes()),
        IconButton(icon: const Icon(Icons.add_circle_outline_rounded, color: Colors.white), onPressed: () { final center = _transformationController.toScene(MediaQuery.of(context).size.center(Offset.zero)); provider.addPeak(center); })
      ]);
  }
  ...
  Widget _buildOutlineOverlay(MountMapProvider provider) {
    ...
    return Container(
      color: provider.currentTheme == AppThemeMode.dark
  ...
  void _showSearch(MountMapProvider p) { showSearch(context: context, delegate: NodeSearchDelegate(nodes: p.nodes, onSelect: (n) => _focusOnNodes(targetPosition: n.position))); }
}
...
class NodeSearchDelegate extends SearchDelegate {
  final List<NodeModel> nodes; final Function(NodeModel) onSelect;
  NodeSearchDelegate({required this.nodes, required this.onSelect});
  @override ThemeData appBarTheme(BuildContext context) => ThemeData.dark().copyWith(scaffoldBackgroundColor: const Color(0xFF0F111A), appBarTheme: const AppBarTheme(backgroundColor: Color(0xFF161821)));
  ...
  Widget _list() { final results = nodes.where((n) => n.text.toLowerCase().contains(query.toLowerCase())).toList(); return ListView.builder(itemCount: results.length, itemBuilder: (context, i) => ListTile(title: Text(results[i].text, style: const TextStyle(color: Colors.white)), onTap: () { onSelect(results[i]); close(context, null); })); }
}
