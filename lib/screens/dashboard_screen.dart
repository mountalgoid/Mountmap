...
class _MountMapDashboardState extends State<MountMapDashboard> {
  ...
  Widget _buildSliverAppBar(MountMapProvider provider) {
    return SliverAppBar(
      ...
      actions: [
        ...
        IconButton(
          icon: Icon(_isSearching ? Icons.close_rounded : Icons.search_rounded, color: Colors.white70, size: 20),
          onPressed: () {
            setState(() {
              ...
              for (int i = 0; i < 7; i++) ...[
                Expanded(
                  child: Center(
                    child: Text(
                      dayLetters[i],
                      style: TextStyle(color: Colors.white.withValues(alpha: 0.3), fontSize: 6.5, fontWeight: FontWeight.bold)
                    ),
                  ),
                ),
          ...
          Expanded(
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Container(
                  decoration: BoxDecoration(
                    color: isActive ? MountMapColors.teal : Colors.white.withValues(alpha: 0.05),
  ...
  Widget _buildBreadcrumbs(MountMapProvider provider) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: MountMapColors.darkCard.withValues(alpha: 0.5),
        ...
        child: Row(
          children: [
            InkWell(
              ...
              child: Icon(Icons.home_rounded, size: 16, color: Colors.white.withValues(alpha: 0.3)),
            ),
            for (var crumb in crumbs) ...[
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 8),
                child: Icon(Icons.chevron_right_rounded, size: 14, color: Colors.white.withValues(alpha: 0.1)),
              ),
              InkWell(
                ...
                child: Text(
                  crumb.title,
                  style: TextStyle(
                    color: crumb == crumbs.last ? MountMapColors.teal : Colors.white.withValues(alpha: 0.5),
                    fontSize: 11,
        ...
        child: Row(
          children: [
            CustomPaint(
              ...
              painter: WorkflowLinePainter(isLast: false, isUp: true),
            ),
            Icon(Icons.arrow_upward_rounded, size: 14, color: Colors.white.withValues(alpha: 0.2)),
            const SizedBox(width: 8),
            Text("Go back", style: TextStyle(color: Colors.white.withValues(alpha: 0.2), fontSize: 11, fontStyle: FontStyle.italic)),
          ],
        ),
      ),
    );
  ...
  Widget _buildWorkflowNode(MindMapAsset asset, MountMapProvider provider, bool isLast) {
    ...
    final Color accentColor = isFolder ? Colors.amber : MountMapColors.teal;
      ...
      children: [
        ...
        Padding(
          ...
          child: CustomPaint(
            ...
            painter: WorkflowLinePainter(isLast: isLast),
          ),
        ),
            ...
            child: Material(
              ...
              child: InkWell(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: isFolder ? Colors.amber.withValues(alpha: 0.03) : MountMapColors.darkCard,
                    borderRadius: BorderRadius.circular(20),
                        ...
                        decoration: BoxDecoration(
                          gradient: LinearGradient(
                            colors: isFolder 
                              ...
                              : [MountMapColors.violet.withValues(alpha: 0.3), MountMapColors.teal.withValues(alpha: 0.1)],
                            begin: Alignment.topLeft,
                            end: Alignment.bottomRight,
                          ),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                Text(
                                  isFolder ? "BRANCH" : "PEAK",
                                  style: TextStyle(color: accentColor.withValues(alpha: 0.6), fontSize: 8, fontWeight: FontWeight.w900, letterSpacing: 1.5),
                                ),
                                const SizedBox(width: 8),
                                Container(width: 2, height: 2, decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.1), shape: BoxShape.circle)),
                                const SizedBox(width: 8),
                                Text(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style: const TextStyle(color: Colors.white24, letterSpacing: 1),
          ),
        ],
      ),
    );
  ...
  PopupMenuItem<String> _menuItem(String title, IconData icon, String value, {bool isDelete = false}) {
    return PopupMenuItem(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        Text(title, style: TextStyle(color: isDelete ? Colors.redAccent : Colors.white70, fontSize: 13)),
      ]),
    );
  }
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                const Text("ADD NEW ITEM", style: TextStyle(color: Colors.white54, letterSpacing: 3, fontSize: 10)),
                IconButton(
            ...
            ListTile(
              ...
              title: const Text("Create MindMap", style: TextStyle(color: Colors.white)),
            ...
            ListTile(
              ...
              title: const Text("Import .mountflow File", style: TextStyle(color: Colors.white)),
              onTap: () {
                Navigator.pop(context);
          ...
          children: [
            Padding(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text("HIERARCHICAL CHART SELECTION",
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text("MOVE ITEM", style: TextStyle(color: Colors.white54, letterSpacing: 4, fontSize: 10)),
            const SizedBox(height: 20),
            ListTile(
              leading: const Icon(Icons.grid_view_rounded, color: Colors.white38),
              title: const Text("Main Directory (Root)", style: TextStyle(color: Colors.white)),
  ...
  void _showDeleteConfirm(BuildContext context, MindMapAsset asset, MountMapProvider provider) {
    _showStyledDialog(
      ...
      content: Text("Delete '${asset.title}' permanently?", style: const TextStyle(color: Colors.white70)),
      actions: [
        TextButton(
          onPressed: () => Navigator.pop(context), 
          style: TextButton.styleFrom(
...
class WorkflowLinePainter extends CustomPainter {
  ...
  WorkflowLinePainter({required this.isLast, this.isUp = false});
