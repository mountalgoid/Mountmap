...
class MountMapProvider extends ChangeNotifier {
  ...
  void autoLayout() {
    ...
    double currentY = 0;
    const double verticalGap = 200.0; // Increased to avoid overlap
    const double horizontalGap = 450.0; // Increased for side tags

    for (var root in roots) {
      double treeHeight = _layoutNodeRecursive(root, 0.0, currentY, verticalGap, horizontalGap);
  ...
  double _layoutNodeRecursive(NodeModel node, double currentX, double startY, double vGap, double hGap) {
    List<NodeModel> children = _nodes.where((n) => n.parentId == node.id).toList();

    // Account for node content in vertical height
    double estimatedNodeHeight = 60.0;
    if (node.description != null && node.description!.isNotEmpty) estimatedNodeHeight += 20.0;
    if (node.labels.isNotEmpty) {
      estimatedNodeHeight += (node.labels.length / 2).ceil() * 25.0 + 10.0;
    }

    double nodeVisualHeight = math.max(vGap, estimatedNodeHeight + 40.0);

    if (children.isEmpty) {
      updatePeak(node.id, position: Offset(currentX, startY + (nodeVisualHeight / 2)));
    ...
    for (var child in children) {
      double childHeight = _layoutNodeRecursive(child, nextX, startY + accumulatedHeight, vGap, hGap);
    ...
    double centeredY = startY + (accumulatedHeight / 2);
